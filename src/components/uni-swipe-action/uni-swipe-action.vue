	...
	<view class="uni-swipe-action">
		<view :class="{'uni-swipe-action--show':isShowBtn}" :style="{'transform':transformX,'-webkit-transform':transformX}" class="uni-swipe-action__container" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd" @touchcancel="touchEnd" @click="bindClickCont">
			...
			<view :id="elId" class="uni-swipe-action__btn-group">
				<div v-for="(item,index) in options" :key="index" :style="{backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : '#fdf3f3',color: item.style && item.style.color ? item.style.color : '#655657',fontSize: item.style && item.style.fontSize ? item.style.fontSize : '28upx',borderColor: item.style && item.style.borderColor ? item.style.borderColor : '#ba0000'}" class="uni-swipe-action--btn" @click="bindClickBtn(item,index)">
					{{ item.text }}
				</div>
			</view>
		</view>
